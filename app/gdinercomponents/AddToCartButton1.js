...
const AddToCartButton1 = React.memo(({ item,selected_mess_code,selected_mess_name }) => {
  const dispatch = useDispatch();
 console.log(item,selected_mess_code,selected_mess_name);
  ...
  const handleCartUpdate = useCallback(
    (type) => {
      ...
      // ✅ Remove mess if no items are left
      if (updatedItems.length === 0) {
        dispatch(updateCart({ mess_code: selected_mess_code, mess_name: selected_mess_name, total: 0, items: [] }));
      } else {
