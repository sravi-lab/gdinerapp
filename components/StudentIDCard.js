...
const StudentIDcard = ({ profileData }) => {
  const [imageUri, setImageUri] = useState("");
  const [encodedid, setEncodedID] = useState();
  const [currentDateTime, setCurrentDateTime] = useState("");

  useEffect(() => {
    if (profileData?.regdno) {
      setImageUri(`https://doeresults.gitam.edu/photo/img.aspx?id=${profileData.regdno}`);
    }
  ...
  useEffect(() => {
    if (encodedid != '') {
      const regdno = profileData?.regdno || "NoQR";
      setEncodedID((btoa('DGID#' + regdno)))
    }
  }, [profileData])
  ...
  return (
    <View style={styles.cardContainer}>
      ...
      <View style={styles.headerSection}>
        ...
        <Text style={styles.studentName}>{profileData?.name || "N/A"}</Text>
      </View>
          ...
          <HStack space={4} alignItems="flex-start">
            ...
            <VStack style={styles.detailsSection} space={0}>
              ...
              <View style={styles.detailRow}>
                <Text style={styles.labelText}>Campus & Branch</Text>
                <Text style={styles.valueText}>{profileData?.campus || "N/A"}, {profileData?.branch_code || "N/A"}</Text>
              </View>
              
              <View style={styles.detailRow}>
                <Text style={styles.labelText}>Batch</Text>
                <Text style={styles.valueText}>{profileData?.batch || "N/A"}</Text>
              </View>
            ...
            <Center style={styles.imageSection}>
              ...
              <View style={styles.idBadge}>
                <Text style={styles.idBadgeText}>{profileData?.regdno || "N/A"}</Text>
              </View>
            </Center>
          </HStack>
        </Box>
